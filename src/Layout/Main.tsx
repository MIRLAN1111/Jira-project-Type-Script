...
const Main: React.FC = () => {
  ...
  const deleteItem = (boardId: number, itemId: number) => {
    const updatedBoards = boards.map((board: Board) => {
      if (board.id === boardId) {
        const updatedItems = board.items.filter((item: Item) => item.id !== itemId);
    ...
    <div className="app">
      {boards.map((board: Board) => (
        <div className="board" key={board.id}>
          ...
          <Button variant="contained" onClick={() => handleButtonClick(board.id)}>
            Добавить задачу
          </Button>
...
const ButtongAddTasks =styled(Button)`
`